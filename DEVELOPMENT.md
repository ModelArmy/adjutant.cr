...
**Default-parameter binding.** `Compiler.compile_proc` takes full `Param` nodes (not bare names) so it can emit a small prologue — one conditional block per param with a `default` — before the body's own bytecode. Each checks `Op::GetArgc` (the actual caller-supplied arg count, pushed by the new `Op::GetArgc` opcode reading `Frame#argc`) against that param's declared slot index; if the caller didn't supply it, the default expression compiles and runs inline (via the ordinary `compile_node` path, so it can reference earlier params — `def add(a, b = a + 1)` — the same way any other expression in the body could) and its result is stored to the param's slot. This lives in the compiler, not the VM, specifically because evaluating an arbitrary default expression means running compiled bytecode, and only the compiler produces that; `VM#bind_args` (below) explicitly leaves the slot at `nil_value` for the prologue to fill in. A `kwarg?` param's `default` is deliberately excluded from this prologue even though it's a `default` too — keyword arguments (call-site `name: value` syntax, required-kwarg enforcement) are a separate, not-yet-implemented piece; see `SCOPE.md`'s Must Fix entry.

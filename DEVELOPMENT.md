...
**Globals and bare calls.** `@globals` now holds only `def`s and explicit constants/classes — a plain variable assignment anywhere (top level, class/module body, method body, block) is a real local via `CompilerScope`/`GetLocal`/`SetLocal`, never `@globals`, so top-level methods and top-level variables are genuinely separate namespaces, matching Ruby. `Op::GetGlobal` handles a bare identifier that resolved to neither a local nor an outer-captured local: it routes through the same `dispatch_call` path a real `name()` call uses, which checks native functions, then `@globals` (for a `def`), then builtins, and raises a script-catchable `NameError` if none match — matching Ruby's actual behavior for an undefined bare identifier, rather than silently returning `nil`.
...
- **No implicit-`self` dispatch for a bare receiverless call inside a class/module/method body.** Real Ruby: a bare `x` (no receiver, no parens even needed) inside a method/class/module body implicitly tries `self.x` if `x` isn't a local — e.g. `module M; def x; 42; end; x; end` calls `M`'s own method `x`. `dispatch_call`'s receiverless path (native functions → `@globals` → builtins) never consults `f.self_val`'s own method table at all; only the explicit-receiver path (`has_receiver: true`, e.g. `obj.foo`) does. Found while writing specs for the 2026-07-15 scoping fix — a bare `x` in that exact example currently raises `NameError` instead of returning `42`. A real, separate dispatch gap, not a scoping one; left as its own follow-up.

...
**Known related gap, not yet fixed:** `next`/`redo` inside a `begin...end while`/`until` (the do-while modifier form) target the wrong position and can infinite-loop — see [SCOPE.md](./SCOPE.md)'s `Must Fix` for the trace; `compile_modifier_while` needs its own small design pass (this loop form's condition sits at the *end*, not the start, so `next`/`redo` may need a third distinct target position) rather than the same fix shape as `compile_while`.

...
`Parser#parse_identifier_or_call` therefore has its own dedicated `Minus`-handling branch (checked before the generic, name-agnostic `arg_follows_no_paren?` fallback, which still exists solely for `parse_raise`'s `raise -1` — a keyword call site with no ambiguity at all, since `raise` can never be a local), guarded by `operand_immediately_follows?` — a check using the parser's own pre-existing one-token lookahead buffer (`@next`, populated by `advance`), comparing `@current`'s column plus its length against `@next`'s column. No `known_local?` involvement at all for this branch; adjacency alone decides it, correctly for every case traced (`eq -1, -1` → call; `n - 1`/`a - b`, spaced either way → binary, regardless of local status). The equivalent fix for unary `+` was drafted alongside this one, found to work, and deliberately NOT shipped in the same arc — parked in `SCOPE.md` pending a broader conversation (prompted by this bug family) about whether the parser should keep reconstructing Ruby's whitespace-sensitivity piecemeal like this, or whether the lexer itself should carry real whitespace/context state instead (mirroring MRI/mruby's own lexer-state approach, which is where these ambiguities are actually resolved in real Ruby).

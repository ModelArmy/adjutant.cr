...
module Adjutant::Builtins
  # Registers `Module#include` — real Ruby's Module instance method,
  # not a keyword (see token.cr's own comment on why `include` was
  # removed from the reserved-word table 2026-08-10). Registered on
  # Module specifically, not Class or Object: every class's own
  # `rclass` is Class, and `Class.superclass == Module` (see
  # Interpreter#bootstrap_core_hierarchy), so registering it once
  # here makes it available from BOTH `class Foo; include Bar; end`
  ...
  # history): this registers the module into `included_modules` —

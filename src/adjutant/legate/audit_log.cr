module Adjutant
  module Legate
    # A single §8.7 audit record — one per verb-boundary decision the
    ...
    #
    # `decision` is one of `:allowed`, `:denied` (Grants static gate),
    # or `:rejected` (RiskFlowPolicy dynamic layer, reached only after
    # `:denied` was ruled out — see broker.cr's own ordering comment).
    # `grant` is the Legate grant category consulted (`:read`,
    # `:write`, `:delete`, `:net`, `:exec`), matching §8.7's "grant
    # consulted" field directly.
    struct AuditRecord
      ...
      def initialize(@verb : String, @subject : String, @grant : Symbol, @decision : Symbol,

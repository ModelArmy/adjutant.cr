...
- **`Legate.read`'s `scrub`/`Malformed` logic doesn't work at all —
  `scrub: true` silently returns un-scrubbed invalid UTF-8, and
  `scrub: false` can never raise `Malformed`.** Found 2026-08-27, in a
  systematic audit of the Legate read-verb slice against LEGATE.md
  (prompted by the project's own "Legate has no upstream Ruby
  behavior to fall back on, so it needs deliberate auditing" framing
  — read.cr had been flagged verbally, in conversation, as likely
  broken back when the identical bug was confirmed and fixed in
  `lines.cr`, but never actually written down anywhere until now).
  `read.cr`'s `read_content` (still) assumes `String.new(Bytes)`
  substitutes invalid UTF-8 with U+FFFD; `ops test` already disproved
  that assumption for the exact same code shape in `lines.cr` — it
  does neither validation nor substitution, just wraps the raw bytes
  unchecked. Two concrete, currently-shipped consequences: (1) §2.6's
  documented default ("invalid byte sequences are replaced with
  U+FFFD") silently doesn't happen — a script reading a file with
  stray invalid bytes gets them back completely unscrubbed; (2)
  `scrub: false`'s `Malformed` detection (`scrubbed.to_slice !=
  raw_bytes`) can never fire, since `scrubbed` is always byte-identical
  to `raw_bytes` — comparing a string to itself. `read_spec.cr` has
  zero scrub tests, so nothing in the existing suite would catch
  either half. Fix shape: already proven correct and merged, in
  `lines.cr`'s `LineIterator#build_line` — swap in the same
  `raw_str.valid_encoding? ? raw_str : (scrub ? raw_str.scrub : raise
  Malformed)` shape, and add the scrub-behavior tests `read_spec.cr`
  never had.

- **Kwarg-validation ordering relative to `broker.authorize_read` is
  inconsistent across the read-verb slice.** Found 2026-08-27, same
  audit pass. `records.cr`'s `format:` and `grep.cr`'s `pattern`/
  `paths` are deliberately validated BEFORE authorizing (a bad kwarg
  is a call-site programmer error, unrelated to the grant — shouldn't
  consume an audit-log entry for a call that's going to fail
  regardless of the path's validity; see records.cr's own comment,
  where this principle was first articulated). `read.cr`/`list.cr`/
  `bytes.cr`/`lines.cr` — all written BEFORE that principle was
  articulated — validate AFTER instead (`scrub_flag`/`limit_of`/
  `chunk_size_of`/`max_line_of` all called post-`authorize_read`).
  Same kind of situation, inconsistent behavior verb-to-verb, purely
  an artifact of write-order rather than a deliberate distinction.
  Must Fix rather than Will Fix specifically because Legate has no
  upstream reference to silently self-correct against — an
  inconsistency here IS a form of incorrect behavior for this
  project's own stated bar, not merely untidy. Fix shape: reorder each
  of the four verbs' kwarg-reader calls to run before their own
  `authorize_read` call, matching records.cr/grep.cr exactly; no
  logic inside the readers themselves needs to change for this part.

- **Every kwarg-reading helper across every Legate verb does an
  UNCHECKED type cast (`given.as_int`/`.as_bool`/`.as_sym`), which
  raises a raw Crystal `TypeCastError` — not a clean, script-catchable
  Adjutant error — on a wrong-typed kwarg value.** Found 2026-08-27,
  surfaced while investigating the ordering issue just above: `Value#
  as_int` is literally `@raw.as(Int64)` (value.cr) — Crystal's own
  unchecked cast, with no Adjutant-level type check ahead of it
  anywhere in any verb's kwarg readers (`limit_of` in list.cr/read.cr/
  grep.cr, `chunk_size_of` in bytes.cr, `max_line_of`/`scrub_flag` in
  lines.cr, `headers_flag_of`/`format_of`'s own symbol check in
  records.cr, `context_of` in grep.cr). Concretely:
  `Legate.read(path, limit: "big")` or `Legate.bytes(path, chunk:
  "lots")` doesn't raise a tidy, rescuable `TypeError` today — it
  crashes with an internal-looking Crystal cast failure, the exact
  "ugly internal crash instead of a clean language-level error"
  failure shape this file's own `dup`/`clone` entry (just above) was
  already judged Must-Fix for. Bigger than the ordering issue above:
  touches every kwarg on all seven verbs, not four files. Fix shape:
  a small SHARED helper (`Legate::Helpers`, alongside `fixed_prefix`)
  that checks `Value#int?`/`#bool?`/`#symbol?` before casting and
  raises a clean, catalog-backed `TypeError` (a new R0xx entry, or
  possibly one shared parameterized entry covering "kwarg given the
  wrong type" generically across every verb/kwarg-name pair) instead
  of letting the raw cast run — then every verb's own kwarg readers
  switch to calling it instead of the bare `.as_int`/`.as_bool` they
  use today.

- **`read_spec.cr` and `records_spec.cr` have no "exactly one audit
  record per invocation" test, unlike the other five verb specs
  (stat/list/bytes/lines/grep all have one).** Found 2026-08-27, same
  audit pass. Behavior looks correct by code review (both verbs make
  exactly one `broker.authorize_read` call), so this is a pure test-
  coverage gap, not a suspected behavior bug — flagged as Must Fix
  alongside the other findings from this same audit pass rather than
  filed separately, since fixing it is a small, mechanical addition
  (copy the pattern the other five specs already use) once the pass

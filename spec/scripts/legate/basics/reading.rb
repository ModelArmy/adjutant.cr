...
# comment.
#
# Fixture paths below are written relative to the REPO ROOT
# (`spec/scripts/legate/basics/fixtures/...`), not relative to this
# script's own location — `test_runner.cr` runs every script with the
# repo root as the process's working directory (the same convention
# `crystal spec`/`ops test` already assumes elsewhere in this
# codebase), so a plain literal string here is simplest and needs no
# `__FILE__`/dirname arithmetic inside the script itself.
FIXTURES = "spec/scripts/legate/basics/fixtures"
...
# No `.sort` needed — `Legate.list` already returns entries in the
# same sorted-by-path order `Dir.glob(...).sort` establishes
# internally (list.cr's own implementation) BEFORE building the
# Array a script sees.

...
# The upstream ISO suite gates a batch of UTF-8-specific assertions
# behind `__ENCODING__ == "UTF-8"` — not a real token here (only
# __FILE__/__LINE__ exist, see token.cr), so using it would break
# this file's LOADING entirely, not just one assertion. Every
# UTF8STRING-gated block below is blocked anyway (see each one's own
# comment), so this constant is unused, but kept as a documented
# false rather than deleted, in case UTF-8-specific coverage is
# picked up later and needs it again.
UTF8STRING = false
...
# --- BLOCKED: String#* (repetition) doesn't exist at any level — no
# ValueOps.op case, no native method (see string.cr's own comment).
...
# --- BLOCKED, same assert('String#[]') continued:
# - `'abc'[1.1]` (Float index coercion) — exec_get_index only handles
#   a plain Integer index, no Float-to-Integer coercion.
# - the 2-arg `[start, length]` form (`'abc'[0, -1]`, `'abc'[1, 2]`)
#   — exec_get_index has no case for a second argument at all, only
#   single Integer and (as of this session) Range.
# - the String-needle form (`'abc'['bc']`, searching for a substring)
#   — same gap, no case for a String index.
# - `assert_raise(TypeError) { 'abc'[nil] }` — exec_get_index falls
#   through to nil for an unrecognized index shape rather than
...
# --- BLOCKED, same assert('String#[] with Range') continued: open-
# ended ranges (`'abc'[4..5]`, `'...'[-3..]`) aren't parseable at
# all — the parser's range-literal production always requires an
# explicit expression on both sides (parse_range in parser.cr calls
# parse_expression unconditionally after the range operator, no
# "endless range" special case), a language-level gap, not something
# this string-slicing fix could address on its own. Dropped those
# specific assertions (`e1`'s `[3..4]` case, `k1`/`k2`'s `[-3..]`
# cases) from the block above rather than leaving them broken.

# --- BLOCKED: needs class_eval/alias_method/remove_method (the eval
# family, U006-excluded) plus subclassing String with a custom
# initialize — neither exists.
...
# --- BLOCKED: String is immutable-position — `[]=` (single-index,
# range, or 2-arg start/length assignment) doesn't exist at any
# level; no Op::SetIndex case for a String target, no native method.
# assert('String#[]=') do
#   # length of args is 1
#   a = 'abc'
...
# --- BLOCKED: same `[]=` gap as above, UTF-8 variant.
...
# --- BLOCKED: String#capitalize! (mutating variant) doesn't exist —
# only the non-mutating #capitalize was added this session.
...
# --- BLOCKED: String#chop/#chop! don't exist.
...
# --- BLOCKED: String#downcase! (mutating) doesn't exist.
...
# --- BLOCKED: String#eql? doesn't exist for any type yet (see
# SCOPE.md — not String-specific).
...
# --- BLOCKED: String#hash doesn't exist for any type yet (see
# SCOPE.md — not String-specific).
...
# --- BLOCKED: __send__ is deliberately excluded (U005), so
# #initialize/#initialize_copy can't be reached this way regardless
# of whether either method itself would otherwise work.
...
# --- BLOCKED: "intern" isn't a registered alias — only "to_sym" is.
# See String#to_sym below for the same behavior under its real name.
...
# --- BLOCKED: UTF-8 byte-vs-character length counting for malformed/
# overlong sequences, and byteslice-dependent assertions (#byteslice
# doesn't exist) — not verified, left blocked rather than guessed.
...
# --- BLOCKED: String#replace (mutates in place) doesn't exist.
...
# --- BLOCKED: UTF-8 reverse correctness (multi-byte characters
# staying intact, not reversed byte-by-byte) wasn't verified against
# Crystal's own String#reverse semantics this session — left blocked
...
# --- BLOCKED: String#reverse! (mutating) doesn't exist.
...
#   # Not implemented yet (upstream's own note — scan needs regex
#   # support, which this project doesn't have at all)
# end
...
# --- BLOCKED: UTF-8 byte-vs-character size counting for malformed/
# overlong sequences, and #bytesize (doesn't exist) — not verified.
...
# --- BLOCKED: String#slice doesn't exist as a distinct method (only
# the `[]` bracket syntax, which itself only supports a single
# Integer index or a Range — see the `[]` block above for those
# gaps).
...
# --- BLOCKED: UTF-8 character-boundary splitting not verified this
# session.
...
# --- BLOCKED: real Ruby's String#to_f/#to_i tolerate underscores as
# digit separators, leading/trailing whitespace, and a base argument
# (`to_i(16)`) — none of which is verified to match here (#to_f/#to_i
# delegate straight to Crystal's own to_f64?/to_i64?, whose exact
# leniency rules for underscores/whitespace/partial-parse weren't
# checked against Ruby's). Left blocked rather than asserted from
# assumption; a plain "123".to_i / "1.5".to_f (no underscores, no
# whitespace, no partial garbage) almost certainly already works but
# isn't in the upstream suite as its own minimal case.
...
# --- BLOCKED: String#upcase! (mutating) doesn't exist.
...
# --- BLOCKED: String#inspect isn't registered as a native method on
# String — the ISO cases here test escape-sequence rendering (`"\0"`
# -> `"\\x00"`) which needs real inspect-specific logic anyway, not
# just Value#inspect's generic String case (which embeds the raw
# byte rather than escaping it).
...
# --- BLOCKED: depends on String#* (`"A" * 32`), which doesn't exist.
# String interpolation itself works fine — this specific regression
# test just happens to need repetition to build its fixture string.
# assert('String interpolation (mrb_str_concat for shared strings)') do
#   a = "A" * 32
#   assert_equal "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA:", "#{a}:"
# end

# --- BLOCKED: #bytes/#each_byte/#getbyte/#setbyte/#byteslice/
# #bytesplice/#byteindex/#byterindex — none of these byte-level
# methods exist; Adjutant's String has no distinct byte-vs-character
# view at all.
...
# --- BLOCKED: adjacent string-literal concatenation ("A" "B" -> "AB")
# isn't verified to be supported by the parser/lexer this session —
